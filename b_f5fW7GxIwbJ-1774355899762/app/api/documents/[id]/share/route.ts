...
function toPublicFilePath(fileUrl: string) {
...
export async function POST(
  ...
  try {
    ...
    const document = await queryOne<{
      ...
      'SELECT id, title, document_number, file_url, file_name, file_type FROM documents WHERE id = $1',
      [id]
    )
    if (!document) {
    ...
    if (send_email) {
      ...
      const attachmentPath = toPublicFilePath(document.file_url)
      try {
