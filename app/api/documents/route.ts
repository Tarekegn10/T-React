...
import { buildReadableDocumentAccessCondition } from '@/lib/permissions'
import { normalizeStoredUploadUrl } from '@/lib/uploads'
import {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const access = buildReadableDocumentAccessCondition('d', user, paramIndex)
    whereConditions.push(access.clause)
    params.push(...access.params)
    paramIndex = access.nextParamIndex
...
export async function POST(request: NextRequest) {
  try {
    ...
    const documentCreationLimit = applySimpleRateLimit({
      key: `documents:create:${getClientIpAddress(request)}`,
      maxRequests: 60,
      windowMs: 10 * 60 * 1000,
    })

    if (!documentCreationLimit.allowed) {
      return createRateLimitResponse(documentCreationLimit.retryAfterSeconds)
    }
    ...
    const normalizedFileUrl = normalizeStoredUploadUrl(file_url)
    ...
    if (file_url && !normalizedFileUrl) {
      return NextResponse.json(
        { error: 'File URL must reference a previously uploaded file' },
        { status: 400 }
      )
    }
    ...
    if (user.role !== 'admin') {
      if (user.department_id) {
        if (resolvedDepartmentId && resolvedDepartmentId !== user.department_id) {
          return NextResponse.json(
            { error: 'You can only create documents inside your own department' },
            { status: 403 }
          )
        }
        ...
        return NextResponse.json(
          { error: 'Only admins can assign documents to a department from this account' },
          { status: 403 }
        )
      }
    }
