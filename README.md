...
- Views for reporting
- Default seed data for departments and system settings
...
The schema no longer creates default login accounts. After you update `.env.local` with your real PostgreSQL credentials, create your first admin user with:

```bash
npm run create-admin -- --username admin --email admin@example.com --password "StrongPassword123!" --name "Administrator"
```

Then sign in with that username or email and the password you chose.
...
| `POST /api/auth/login 500` with user `username` | `.env.local` still has placeholder `DATABASE_URL`; replace it with your real PostgreSQL login |
